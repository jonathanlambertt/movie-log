...
// rating beneath it.
//
// The rating chip reads as a score rather than a stray number: a filled heart
...
// the heart alone. (Earlier passes colored the digit itself, which is hard to
...
export function FilmRow({
  ...
  return (
    ...
    >
      {day ? (
        <Text className="w-7 text-center text-xl font-bold text-text-muted">
          {day}
        </Text>
      ) : null}
        ...
        {rating != null || badge ? (
          <View className="mt-1.5 flex-row items-center gap-2">
            {rating != null ? (
              ...
              <View className="flex-row items-baseline gap-1 rounded-md py-0.5 pl-1.5 pr-2">
                <Heart
                  size={11}
                  color={ratingColor(rating, resolved)}
                  fill={ratingColor(rating, resolved)}
                  // Baseline alignment works on the text, not the SVG box, so
                  // nudge the heart down onto the digits' baseline.
                  style={{ alignSelf: "center", marginTop: 1 }}
                />
                <Text className="text-medium font-bold text-text-primary">
                  {rating}
                </Text>
                <Text className="text-sm font-medium text-text-faint">/10</Text>
              </View>
            ) : null}
            {badge}
          </View>
